...
def _create_minimal_test_tree(root: Path) -> None:
    for directory in [
        "tests/core", "tests/framework", "tests/framework/adapters",
        "tests/framework/adapters/stabilization", "tests/modules",
        "tests/services", "tests/sdk", "tests/tools",
...
def test_test_integrity_passes_for_complete_test_tree(tmp_path: Path):
    ...
    for source_dir in ["framework/adapters", "sdk", "tools/repository_integrity"]:
...
def test_test_integrity_detects_missing_test_files(tmp_path: Path):
    _create_minimal_test_tree(tmp_path)
    (tmp_path / "sdk").mkdir(parents=True, exist_ok=True)
