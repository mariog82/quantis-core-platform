...
class InMemoryAIAdapter(AIAdapter):
    ...
    def complete(self, request: AIRequest) -> AIResponse:
        user_messages = [message.content for message in request.messages if message.role == AIRole.USER]
    ...
    def embed(self, request: AIEmbeddingRequest) -> AIEmbeddingResponse:
        ...
        for index, item in enumerate(inputs):
            values = [float(len(item)), float(sum(ord(char) for char in item) % 997), float(index)]
