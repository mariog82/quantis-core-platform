...
REQUIRED_WORKFLOWS = [
    ...
    ".github/workflows/m5-1-ci-stabilization.yml",
]
    ...
    for workflow in REQUIRED_WORKFLOWS:
        ...
        if not workflow_path.exists():
            report.add_issue(
                "MISSING_WORKFLOW",
                workflow,
                "Required CI workflow is missing.",
            )
            continue

        missing_patterns = _workflow_missing_required_patterns(workflow_path)
        for pattern in missing_patterns:
            report.add_issue(
                "MISSING_CI_PATTERN",
                workflow,
                f"Required CI pattern `{pattern}` is missing.",
            )
