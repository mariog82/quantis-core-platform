    ...
    for package in PUBLIC_PACKAGES:
        ...
        if not package_path.exists():
            report.add_issue(
                "MISSING_PACKAGE",
                package,
                "Required public package directory is missing.",
            )
            continue
        ...
        if _is_namespace_package(package_path):
            report.add_issue(
                "MISSING_INIT",
                package,
                "Package is missing __init__.py and may become a namespace package.",
            )

        if _contains_only_pycache(package_path):
            report.add_issue(
                "PYCACHE_ONLY_PACKAGE",
                package,
                "Package contains only __pycache__.",
            )
